...
class runConfirmation:
    def __init__(self):
        ...
        # Title the Element
        title_label = Label(self.base_menu, text="Confirmation", font=("Comic Sans", 16))
        title_label.grid(row=0, column=3, padx=25, sticky="nsew")

        # Create the "Start" button
        run_button = Button(self.base_menu, text="Start", width=10, height=5, command=self.run)
        run_button.grid(row=2, column=2, sticky="sew")

        # Create the "Stop/Exit" button
        run_button = Button(self.base_menu, text="Stop/Exit?", width=10, height=5, command=self.run)
        run_button.grid(row=3, column=2, sticky="sew")

        # Create the "Main Menu" button
        main_menu_button = Button(self.base_menu, text="Main Menu", width=10, height=2, command=self.run)
        main_menu_button.grid(row=4, column=1, pady=10, sticky="sew")
